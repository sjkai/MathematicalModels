...
iter=1e8;               %内部蒙特卡洛循环迭代次数
%随机初始化城市坐标
city=struct([]);
for i=1:n
    % city(i).x=floor(1+100*rand()); 
    % city(i).y=floor(1+100*rand());
    city(i).x=randi(100,1,1);%floor(1+100*rand()); 
    city(i).y=randi(100,1,1);%floor(1+100*rand());
end
load city


OptFunc=@computer_tour;
ArriseNew=@perturb_tour;
l=1;                            %统计迭代次数
len(l)=OptFunc(city,n);   %每次迭代后的路线长度  
netplot(city,n);                %初始旅行路线

while temperature>1e-5    %停止迭代温度
    
    for i=1:iter     %多次迭代扰动，一种蒙特卡洛方法，温度降低之前多次实验
        len1=OptFunc(city,n);         %计算原路线总距离
        tmp_city=ArriseNew(city,n);      %产生随机扰动
        len2=OptFunc(tmp_city,n);     %计算新路线总距离
